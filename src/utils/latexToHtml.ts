...
export function convertLatexToHtml(latexContent: string): LaTeXConversionResult {
  ...
  try {
    ...
    html = html.replace(/\\section\*?\{(.*?)\}/g, '<h2 class="text-xl font-bold mt-6 mb-2 border-b border-gray-300 pb-1">$1</h2>');
    ...
    // Handle itemize with options (FIXED)
    ...
    // Wrap the result in a div with resume styling (IMPROVED)
    html = `<div style="
